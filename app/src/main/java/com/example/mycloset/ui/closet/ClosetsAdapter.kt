...
class ClosetsAdapter(
    private val onClick: (Closet) -> Unit = {}
) : ListAdapter<Closet, ClosetsAdapter.VH>(DIFF) {

    companion object {
        private val DIFF = object : DiffUtil.ItemCallback<Closet>() {
            override fun areItemsTheSame(oldItem: Closet, newItem: Closet) = oldItem.id == newItem.id
            override fun areContentsTheSame(oldItem: Closet, newItem: Closet) = oldItem == newItem
        }
    }
